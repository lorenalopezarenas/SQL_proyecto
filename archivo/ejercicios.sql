...
select 
	...
	count (c.category_id) as recuento
...
select round(AVG(extract(day from (r.return_date - r.rental_date))), 2) || ' días' as duracion_media_alquiler 
from rental r ;
...
where f.rental_duration > 8
  -- Ninguna película ha sido alquilada por más de 8 días 
